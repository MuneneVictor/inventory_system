...
$notFoundSerials = [];
$singlePrinter = null;
$availableBranches = ['KIMATHI', 'MOI']; // Hardcoded branches
$current_branch = null;
...
// Process transfer printer search
if(isset($_POST['search_serial'])) {
    ...
    if(empty($input)) {
        ...
        $error = "Please enter the name of the person delivering the printers.";
    } else {
        // Check if user has permission to transfer from selected branch
        if(!$is_super_admin && $from_branch !== $current_branch) {
            $error = "You can only transfer printers from your own branch.";
        } else {
            ...
            if(empty($serials)) {
                ...
                // Build query for printers table
                $stmt = $conn->prepare("
                    SELECT *
                    FROM printers
                ...
                if(empty($foundPrinters)) {
                    $error = "Printers not found in selected branch or not in stock.";
...
// Process single printer transfer
if(isset($_POST['transfer_printer'])) {
    ...
    if(!$from_branch || !$to_branch) {
        ...
        if(!$is_super_admin && $from_branch !== $current_branch) {
            ...
            // Verify printer exists and is in the correct branch
            $stmt = $conn->prepare("
                SELECT * 
                FROM printers
            ...
            if($printer) {
                ...
                $update = $conn->prepare("UPDATE printers SET branch = :to_branch WHERE serial_number = :sn");
                $update->execute(['to_branch' => $to_branch, 'sn' => $serial]);
                
                // Log the transfer with delivered_by information and serial number
                $log = $conn->prepare("
                    INSERT INTO activity_logs (user_id, action, details) 
                    VALUES (:uid, 'Transfer printer', :details)
                ");
                $log->execute([
                    'uid' => $_SESSION['user_id'], 
                    'details' => "Transferred printer SN: {$printer['serial_number']} from $from_branch to $to_branch (Delivered by: $delivered_by)"
                ]);
                
                $success = "Printer transferred successfully from $from_branch to $to_branch! (Delivered by: $delivered_by)";
                $singlePrinter = null;
                $foundPrinters = [];
                ...
                $error = "Printer not found in selected branch or already sold.";
            }
        }
    }
}

// Process bulk printers transfer
if(isset($_POST['transfer_bulk_printers'])) {
    ...
    if(empty($selectedSerials)) {
        $error = "No printers selected for transfer.";
    } elseif(!$from_branch || !$to_branch) {
        $error = "Branch information missing. Please search again.";
    } elseif(empty($delivered_by)) {
        $error = "Delivery information missing. Please search again.";
        ...
        if(!$is_super_admin && $from_branch !== $current_branch) {
            ...
            foreach($selectedSerials as $serial) {
                ...
                if($printer) {
                    ...
                    $log->execute([
                        'uid' => $_SESSION['user_id'], 
                        'details' => "Transferred printer SN: $serial from $from_branch to $to_branch (Delivered by: $delivered_by)"
                    ]);
                } else {
                    $failedCount++;
                }
            ...
            if($transferredCount > 0) {
                ...
                $log->execute([
                    'uid' => $_SESSION['user_id'], 
                    'details' => "Bulk transfer: $transferredCount printer(s) [SN: $serialList] from $from_branch to $to_branch (Delivered by: $delivered_by)"
                ]);
                
                $success = "$transferredCount printer(s) transferred successfully from $from_branch to $to_branch! (Delivered by: $delivered_by)";
                if($failedCount > 0) {
                    $success .= " $failedCount printer(s) could not be transferred.";
                }
                $foundPrinters = [];
                $notFoundSerials = [];
                ...
                $error = "No printers could be transferred.";
            }
        }
    }
}
...
<form method="POST" id="transferForm">
    <div class="branch-selector">
        <div>
            ...
            <?php if(!$is_super_admin): ?>
                <div class="branch-note">You can only transfer printers from: <strong><?= htmlspecialchars($current_branch) ?></strong></div>
    ...
    <div class="delivery-info">
        ...
        <input type="text" name="delivered_by" id="delivered_by" placeholder="Enter the name of person delivering printers" value="<?= isset($_POST['delivered_by']) ? htmlspecialchars($_POST['delivered_by']) : (isset($_SESSION['delivered_by']) ? htmlspecialchars($_SESSION['delivered_by']) : '') ?>" required>
    </div>
    ...
    <button type="submit" name="search_serial">Search Printer(s)</button>
</form>
...
<form method="POST">
    <h3>Printer Details - Ready to Transfer</h3>
    ...
    <button type="submit" name="transfer_printer">Confirm Transfer</button>
</form>
<?php endif; ?>

<?php if(!empty($foundPrinters)): ?>
<form method="POST">
    <h3>Found Printers (<?= count($foundPrinters) ?>) - Ready to Transfer</h3>
    <?php if(isset($_SESSION['transfer_to_branch'])): ?>
    ...
    <table>
        ...
        <?php foreach($foundPrinters as $index => $printer): ?>
        <tr>
            <td class="checkbox-cell"><?= $index + 1 ?></td>
            <td class="checkbox-cell"><input type="checkbox" name="selected_serials[]" value="<?= htmlspecialchars($printer['serial_number']) ?>" checked></td>
            <td><?= htmlspecialchars($printer['serial_number']) ?></td>
            <td><?= htmlspecialchars($printer['model_name']) ?></td>
            <td><?= htmlspecialchars($printer['status']) ?></td>
            <td><?= htmlspecialchars($printer['branch']) ?></td>
        </tr>
        <?php endforeach; ?>
    </table>
    <button type="submit" name="transfer_bulk_printers">Transfer Selected Printers (<?= count($foundPrinters) ?>)</button>
</form>
<?php endif; ?>
